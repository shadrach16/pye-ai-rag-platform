...
def extract_metadata_from_pdf(pdf_file_path: str) -> Dict[str, str]:
    with open(pdf_file_path, "rb") as pdf_file:
        ...
        return {
            ...
            "creation_date": getattr_or_default(metadata, 'creation_date', date(1900, 1, 1)).strftime('%Y-%m-%d'),
        }

def extract_metadata_from_txt(txt_file_path: str) -> Dict[str, str]:
    # Extract metadata from a TXT file if applicable
    return {}

def extract_metadata_from_csv(csv_file_path: str) -> Dict[str, str]:
    # Extract metadata from a CSV file if applicable
    return {}

def extract_metadata_from_json(json_file_path: str) -> Dict[str, str]:
    # Extract metadata from a JSON file if applicable
    return {}

def extract_metadata_from_xml(xml_file_path: str) -> Dict[str, str]:
    # Extract metadata from an XML file if applicable
    return {}

def extract_metadata_from_docx(docx_file_path: str) -> Dict[str, str]:
    doc = docx.Document(docx_file_path)
    metadata = {
        "title": doc.core_properties.title.strip(),
        "author": doc.core_properties.author.strip(),
        "creation_date": doc.core_properties.created.strftime('%Y-%m-%d'),
    }
    return metadata

def extract_metadata_from_xlsx(xlsx_file_path: str) -> Dict[str, str]:
    wb = openpyxl.load_workbook(xlsx_file_path)
    metadata = {
        "title": wb.properties.title.strip(),
        "author": wb.properties.creator.strip(),
        "creation_date": wb.properties.created.strftime('%Y-%m-%d'),
    }
    return metadata
